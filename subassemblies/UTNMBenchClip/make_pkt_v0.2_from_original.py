"""
Generate UTNMBenchClip.pkt from UTNMBench (UTNMBench-C3D25-DrainConnect.pkt), adding the bowtie clip targets.
...
numbers, codes, expressions, decisions), and the only changes are:
  ...
  * One repair of the source: P290 (Terminal_Bench_5 / Toe Drain marker) had no From point (= subassembly origin);
    its five siblings start at the end of their bench, so P290 now starts at P233.
...
def to_ir(root):
    ...
    def node_ir(el, cont):
        ...
        n = el.get(X + "Name")
        if n in used:
            raise ValueError(f"flow node {n} is reached twice: shared nodes are not handled")
        if n:
            used.add(n)
...
SEQ = {"ap": 10, "v": 300}     # new names stay within three digits and clear of the source (P1-P294, L131-L314, AP1-AP7)


def add_clip(node):
    """Walk the IR; put a clip decision in front of every point that makes a link."""
    if node is None:
        return None
...
def build_xaml(src):
    ...
    flow = ir.next
    # repair: P290 without a From point
    for aid, t in list(texts.items()):
        if 'PointNumber="P290"' in t:
            assert 'FromPoint="{x:Null}"' in t
            texts[aid] = t.replace('FromPoint="{x:Null}"', 'FromPoint="P233"')
